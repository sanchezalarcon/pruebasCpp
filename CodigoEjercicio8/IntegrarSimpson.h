...
#ifndef __INTEGRARSIMPSON_H
#define __INTEGRARSIMPSON_H

class IntegrarSimpson{
    private:
           float Gamma(float x);
           float F(float x, float dof);
           float SumatoriaF(float ini, float n,float k,float w, float dof);
   public:
           IntegrarSimpson();
           ~IntegrarSimpson();
           float Calcular(float xi, float xf, float dof);
